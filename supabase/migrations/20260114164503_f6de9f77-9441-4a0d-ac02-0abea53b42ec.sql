-- Cette migration garantit que user_roles existe avant toute référence
...
-- Créer la table user_roles si elle n'existe pas (AVANT les fonctions qui y font référence)
...
-- Recréer la fonction has_role (maintenant que la table existe)
...
-- Créer les autres fonctions helper
...
RETURNS TABLE (user_id UUID, email TEXT, created_at TIMESTAMPTZ, role TEXT, role_id UUID)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  ...
  END IF;
  RETURN QUERY
  SELECT au.id as user_id, au.email::TEXT, au.created_at, COALESCE(ur.role::TEXT, 'charge_affaires') as role, ur.id as role_id
  FROM auth.users au LEFT JOIN public.user_roles ur ON ur.user_id = au.id ORDER BY au.created_at DESC;
END;
$$;

-- Politiques RLS (avec DROP IF EXISTS pour éviter les doublons)
